...
df=pd.read_csv("E:/Ramya/brushUps/Basics/CASE_STUDIES/GOOD_READS/Data/worstBooksdetails.csv",error_bad_lines = False)
missingYear=[]
...
df['year']=df['year'].apply(strtoInt)
emptyyear_df=df.loc[(df['year']==0)]
df.drop(emptyyear_df.index,inplace=True)
...
plt.legend()
plt.tight_layout()
plt.savefig("E:/Ramya/brushUps/Basics/CASE_STUDIES/GOOD_READS/Data_Analysis/AnalysisPlots/pgsBooksVsYearWorstBooks")
plt.show()
